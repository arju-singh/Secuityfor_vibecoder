...
**Auth security:** weak password hashing (md5/sha1 instead of bcrypt/argon2) · plaintext password comparison · JWT `none` algorithm · hardcoded JWT secret · JWT/token in `localStorage`/`sessionStorage` · weak/low-entropy tokens (`randomBytes(<16)` or truncated randomness) · plaintext API/access-key comparison · login route without brute-force protection (recognizes both rate-limit libraries **and** custom failed-attempt lockout). **Positive affirmations** (good practices detected, info-level): strong password hashing (bcrypt/argon2/scrypt) · hardened cookie (httpOnly + secure + sameSite) · cryptographically-secure token generation · brute-force lockout present on auth.
